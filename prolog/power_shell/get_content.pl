...
os:app_property(power_shell:get_content(Spec, _), path(path(powershell))) :-
    absolute_file_name(Spec, _, [access(read)]).
os:app_property(power_shell:get_content(_, _), argument('Get-Content')).
os:app_property(power_shell:get_content(Spec, _), argument(Argument)) :-
...
os:app_property(power_shell:get_content(_, Options), argument(Argument)) :-
    member(Option, Options),
    option_argument(Option, Argument).
os:app_property(power_shell:get_content(_, Options), option(encoding(Encoding0))) :-
