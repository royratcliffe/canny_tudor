...
\subsection{SWI-Prolog extensions}

This includes the following.

\subsubsection{Non-deterministic `dict_member(?Dict, ?Member)`}
...
Uses \href{https://semver.org/}{Semantic Versioning}.

\subsection{[0.7.2] - 2020-07-25}
...
\section{library(canny/maths)}

\label{sec:maths}

\begin{description}
    \predicate[det]{remainder}{3}{+X:number, +Y:number, -Z:number}
...
\section{library(canny/permutations)}
...
\section{library(canny/situations)}
...
Unifies with \arg{Current} for \arg{Situation} and \arg{When} it happened. Unifies
...
\section{library(canny/situations_debugging)}

\label{sec:situationsdebugging}
...
\section{library(canny_tudor)}

\label{sec:cannytudor}

\section{library(docker/random_names)}

\label{sec:randomnames}
...
\section{library(linear/algebra): Linear algebra}
...
\section{library(os/apps): Operation system apps}
...
\section{library(os/apps_debugging)}

\label{sec:appsdebugging}

\section{library(os/apps_testing)}

\label{sec:appstesting}

\section{library(os/file_searches): File searches}

\label{sec:filesearches}
...
which Prolog defines by default.\vspace{0.7cm}

\section{library(os/lc)}
...
\section{library(os/search_paths)}
...
\section{library(power_shell/get_content)}

\label{sec:getcontent}

\section{library(print/(table))}
...
\section{library(random/temporary)}
