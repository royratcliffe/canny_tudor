...
\chapter{library(canny/clamp)}\label{sec:clamp}

\begin{description}
    \predicate[det]{clamp}{4}{+Min, +Max, +Unclamped, -Clamped}
Clamp a value between a minimum and maximum.

This predicate takes an input value (\arg{Unclamped}) and ensures that it falls
within the specified minimum (\arg{Min}) and maximum (\arg{Max}) bounds. If \arg{Unclamped} is
less than \arg{Min}, \arg{Clamped} will be unified with \arg{Min}. If \arg{Unclamped} is greater
than \arg{Max}, \arg{Clamped} will be unified with \arg{Max}. If \arg{Unclamped} is between \arg{Min} and
\arg{Max}, \arg{Clamped} will be unified with \arg{Unclamped}. The predicate is deterministic
...
\arg{Clamped} & The result of clamping \arg{Unclamped} between \arg{Min} and \arg{Max}. \\
\end{arguments}
\end{description}
...
\chapter{library(canny/csv): Canny CSV}\label{sec:csv}
...
manner using Prolog engines. The main predicate, \predref{csv_read_file_by_row}{3},
allows for non-deterministic reading of CSV rows, yielding one row at a time.\vspace{0.7cm}

\begin{description}
    \predicate[nondet]{csv_read_file_by_row}{3}{+Spec, -Row:list, +Options}
Extracts records from a CSV file, using the given read \arg{Options}. The
resulting \arg{Row} terms have fields named after the CSV header columns
like an options list.

This predicate uses a Prolog engine to read the CSV file
non-deterministically, yielding one \arg{Row} term at a time. This is
...
\arg{Options} & are passed to \predref{csv_read_file_row}{3}. \\
\end{arguments}
\end{description}
...
\chapter{library(canny/roman): Roman Numerals}\label{sec:roman}

Fundamentally, the Roman "number" represents a numerical sum using 9, 5,
4 and 1 as the principal factors. \verb$I=1$, \verb$V=5$ and \verb$X=10$. Prefix \arg{V}
and \arg{X} with \arg{I} to represent \verb$4$ and \verb$9$, respectively. The same
pattern recurs for \verb$L=50$, \verb$C=100$, \verb$D=500$ and \verb$M=1000$.

In \href{https://www.metalevel.at/prolog/dcg}{definite-clause grammar} terms
...
This is a recursive phrase. For every \verb$Number>0$, there is a sum
\verb$Number=Number0+Number1$ where \arg{Number0} is a Roman numeral, \textbf{and}
\arg{Number1} is the sum of subsequent Roman numerals. Finally, no Roman
numeral \verb$[]$ corresponds to \verb$0$. The initial clause applies \href{https://www.swi-prolog.org/man/clpfd.html}{arithmetic
...
The backtracking logic has an interesting side effect. It finds \textbf{all}
...
\begin{enumerate}
    ...
    \item \verb$IV+I=V$
    \item \verb$I+IV=V$
    \item \verb$I+I+I+I+I=V$
\end{enumerate}

Clause order matters for the \dcgref{roman_numeral}{1} predicate; big numbers
...
It helps, therefore, to wrap the grammar using a cut (\verb$!$). The
...
The Roman term, \arg{A} in the previous query, unifies with a list of
...
The representation for 0 is \textbf{logically} blank. That makes total sense,
come to think about it. Subtly, the unification does not \textbf{fail}. It
...
likewise.\vspace{0.7cm}

\begin{description}
    \predicate[semidet]{roman_number}{2}{?Roman:codes, ?Number:integer}
Wrap the \arg{Roman} numerals grammar using a cut. The predicate concludes
the search for a solution upon finding the first, which is the sum
with the largest factors.

\begin{arguments}
\arg{Roman} & A list of character codes representing the \arg{Roman} numeral. \\
\arg{Number} & An integer corresponding to the \arg{Roman} numeral. \\
\end{arguments}

    \dcg[nondet]{roman_numerals}{1}{?Number:integer}
Tail-recursively unifies with a Roman numeral phrase. For every
number greater than 0, there is a sum where \arg{Number} = Number0 +
...
The base case handles the situation where \arg{Number} is 0, resulting in
an empty list.
...
\chapter{library(canny/url)}\label{sec:url}

\begin{description}
    \dcg[nondet]{url_encoded}{1}{?Code}
Similar to \predref{uri_encoded}{3} but simpler.

URL encodes a \arg{Code}. Uses the C-symbol code type (including letters,
...
\end{arguments}
    ...
    \predicate[semidet]{url_coded}{2}{?Decoded, ?Encoded}
Encodes or decodes a sequence of codes. Uses \dcgref{url_encoded}{1} to
perform the encoding or decoding. Succeeds once.

Examples:

\Sdirective{} \verb$url_coded(`Hello_World!`, Encoded)$.
\arg{Encoded} = \arg{Hello_World\%21}.
\Sdirective{} \verb$url_coded(Decoded, `Hello_World%21`)$.
\arg{Decoded} = `Hello_World!`.
...
\chapter{library(redis/tilde): Redis Tilde Operations}\label{sec:tilde}
...
using the tilde syntax. The \verb$~>$ operator is used to send a request
to the Redis server and retrieve a reply, while the \verb$~$ operator is
used to send a pipeline of requests.

The Redis server to use for these operations can be configured using
the \const{server} setting. By default, it is set to \const{default}, but it can
...
The Redis \href{https://redis.io/docs/latest/commands/time}{TIME} command
...
this information and calculates the total time in seconds.\vspace{0.7cm}

\begin{description}
    \predicate[semidet]{\Stilde{}$>$}{2}{+Request, -Reply}
...
\end{arguments}
    ...
    \predicate[det]{\Stilde}{1}{+PipeLine}
...
\chapter{library(redis/xgroup): Redis XGROUP CREATE command wrapper}\label{sec:xgroup}

Wraps Redis's `XGROUP CREATE` command with \predref{xgroup_create}{3} and
\predref{xgroup_create}{4}. Translates Prolog options into Redis command arguments,
extracts stream \predref{id}{1} (default \verb|$|), handles \verb$mk_stream(true)$ and
\predref{entries_read}{1}, and captures optional reply. Behaves idempotently by
catching \arg{BUSYGROUP} errors.\vspace{0.7cm}

\begin{description}
    \predicate[det]{xgroup_create}{3}{+Redis:atom, +Key:atom, +Group:atom}
\nodescription
    \predicate[det]{xgroup_create}{4}{+Redis:atom, +Key:atom, +Group:atom, +Options:list}
Creates a consumer group for a \arg{Redis} stream if it does not already exist.
This predicate attempts to create a consumer group named \arg{Group} for the
stream identified by \arg{Key} in the \arg{Redis} instance specified by \arg{Redis}. If the
consumer group already exists, the predicate succeeds without error.

The predicate can be called with or without the \arg{Options} argument. If \arg{Options}
is not provided, it defaults to an empty list. The \arg{Options} argument allows
...
\arg{Redis} instance "myredis" using:
...
\begin{arguments}
\arg{Redis} & The \arg{Redis} instance identifier. \\
\arg{Key} & The \arg{Redis} stream key. \\
...
\begin{itemize}
    \item \verb$id(Id)$ specifies the ID for the consumer group.
Defaults to '\$' (the latest entry in the stream).
    \item \verb$mk_stream(true)$ specifies that the stream should be created
if it does not already exist.
    \item \verb$entries_read(EntriesRead)$ specifies the number of entries to read
when the group is created.
    \item \verb$reply(Reply)$ specifies a variable to unify with the command's reply.
...
\chapter{library(redis/xtrim): Redis XTRIM command wrapper}\label{sec:xtrim}
...
using the XTRIM command. The \predref{xtrim_using_entry_id}{4} predicate trims
a Redis stream based on a specified entry ID and a window of time,
while the \predref{xtrim_window_minid}{3} predicate calculates the minimum ID
...
\const{window} setting. By default, it is set to 5 seconds, but it can be
...
\begin{description}
    \predicate[det]{xtrim_using_entry_id}{4}{+Redis, +Key, +Id, +Options}
Trim the \arg{Redis} stream identified by \arg{Key} to remove entries older than
the specified \arg{Id} as the upper bound and a window of time defined by
the \predref{window}{1} option. The \arg{Id} is expected to be in the format
"Millis-Sequence", where Millis is the timestamp in milliseconds.
The window size can be specified in \arg{Options}, and if not provided,
the default window size from the window setting will be used.

\begin{arguments}
\arg{Redis} & The \arg{Redis} connection. \\
\arg{Key} & The \arg{Redis} stream key to trim. \\
...
\end{arguments}
    ...
    \predicate[det]{xtrim_window_minid}{3}{+Id, -MinId, +Options}
Calculate the minimum ID for trimming based on the provided entry ID
and the \predref{window}{1} option. The \arg{MinId} is calculated by subtracting
the window size (in seconds) from the timestamp part of the
provided \arg{Id}. The resulting \arg{MinId} is formatted as "Millis-0" to be
...
\chapter{library(swi/lists): Extensions for SWI-Prolog Lists}\label{sec:lists}
...
\chapter{library(threaded/throttle): Threaded Throttle}\label{sec:throttle}
...
timeout values for specific keys and fields.\vspace{0.7cm}

\begin{description}
    \predicate[det]{consume_threaded_throttle}{2}{+Key:atom, +Field:atom}
Listens to a Redis stream for throttle events on the specified \arg{Key}
and \arg{Field}. When an event is received, it creates a thread to handle
...
\end{arguments}
    ...
    \predicate[det]{unconsume_threaded_throttle}{2}{+Key:atom, +Field:atom}
Stops listening to the Redis stream for throttle events on the
specified \arg{Key} and \arg{Field}.
...
\end{arguments}
    ...
    \predicate[det]{current_threaded_throttle_timeout}{4}{+When:atom, +Key:atom, +Field:atom, -Timeout:number}
Retrieves the current timeout value for the specified \arg{Key} and \arg{Field}
based on the \arg{When} condition (idle or wait). If a specific timeout
has been set for the \arg{Key} and \arg{Field}, it will be used; otherwise, the
...
\end{arguments}
    ...
    \predicate[det]{set_threaded_throttle_timeout}{4}{+When:atom, +Key:atom, +Field:atom, +Timeout:number}
Sets a specific timeout value for the specified \arg{Key} and \arg{Field} based
on the \arg{When} condition (idle or wait). This allows for dynamic
